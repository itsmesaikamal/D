...
const DViewOrders = () => {
  ...
  const handleFormDataChange = (field, value) => {
    // Check if the field is numeric or text
    const isNumericField = ['discount', 'tax'].includes(field);
    
    // For numeric fields, ensure the value is non-negative
    const numericValue = isNumericField ? Math.max(parseFloat(value) || 0, 0) : value;
  ...
  const handleGenerateInvoice = async () => {
    try {
      await addDoc(collection(db, 'district-invoice'), { ...formData, items });
